...
class ListsSuite extends FunSuite {
  ...
  test("sum of null input equals zero") {
    assert(sum(null) === 0)
  }
  test("sum of empty input equals zero") {
    assert(sum(List()) === 0)
  }
  test("sum of only number equals that number") {
    assert(sum(List(123432)) === 123432)
  }
  test("sum of two zeros equals zero") {
  ...
  test("sum of both negative and positive numbers") {
    assert(sum(List(-1, -2, 3, 1)) === 1)
  }
  test("sum of repeated both negative and positive numbers") {
    assert(sum(List(-1, -1, 0, 0, 5, 5)) === 8)
  }
  test("sum of edge min & max values") {
  ...
  test("max of null input throws exception") {
    intercept[NoSuchElementException] {
      max(null)
    }
  }
  test("max of empty input throws exception") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }
  test("max of only number equals that number") {
    assert(max(List(123432)) === 123432)
  }
  test("max of two zeros equals zero") {
    assert(max(List(0, 0)) === 0)
  }
  test("max of only negative numbers") {
    assert(max(List(-1, -2)) === -1)
  }
  test("max of both negative and positive numbers") {
    assert(max(List(-1, -2, 3, 1)) === 3)
  }
  test("max of repeated both negative and positive numbers") {
    assert(max(List(-1, -1, 0, 5)) === 5)
  }
  test("max of edge min & max values") {
    assert(max(List(Int.MinValue, Int.MaxValue)) === Int.MaxValue)
  }
